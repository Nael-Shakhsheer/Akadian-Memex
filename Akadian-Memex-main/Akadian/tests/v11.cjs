...
(async()=>{
 ...
 await p.goto('http://localhost:8765/?capture=voice');await p.locator('#introDialog').waitFor();assert(await p.locator('#introShortcut').inputValue());assert(!await p.evaluate(()=>!!recorder));
