...
(async()=>{
 ...
 await p.goto('http://localhost:8765/ideas.html');await p.locator('#introNext').click();await p.locator('#introNext').click();await p.locator('#introNext').click();
 ...
 await p.evaluate(()=>MemexStorage.put({id:'audio-test',text:'',createdAt:Date.now(),audio:new Blob(['test audio'],{type:'audio/wav'}),status:'pending-transcript'}));
 await p.locator('#settingsBtn').click();await p.locator('#lockPass').fill('test passphrase long');await p.locator('#lockConfirm').fill('test passphrase long');await p.locator('#enableLock').click();await p.waitForFunction(()=>MemexStorage.lockedEnabled);
 const raw=await p.evaluate(async()=>{const d=await new Promise(res=>{const r=indexedDB.open('memex');r.onsuccess=()=>res(r.result)});return new Promise(res=>{const r=d.transaction('ideas').objectStore('ideas').getAll();r.onsuccess=()=>res(JSON.stringify(r.result));});});assert(!raw.includes('solar'));assert(!raw.includes('Second idea'));assert.equal(await p.evaluate(()=>localStorage.getItem('memex.settings')),null);
 ...
 assert.equal(await p.evaluate(()=>settings.apiKey),'fake-test-key');assert.equal(await p.locator('.idea-item').count(),3);assert.equal(await p.evaluate(async()=>await (await MemexStorage.all()).find(i=>i.id==='audio-test').audio.text()),'test audio');
 // Authenticated encryption rejects tampering and wrong passphrases.
 assert(await p.evaluate(async()=>{const a=await MemexStorage.encryptBackup({hello:'world'},'long backup password');let rejected=false;try{await MemexStorage.decryptBackup(a,'wrong')}catch{rejected=true;}return rejected&&(await MemexStorage.decryptBackup(a,'long backup password')).hello==='world';}));
 // Imported links cannot execute JavaScript; malformed data fails before writes.
 assert(await p.evaluate(()=>{const a=validateBackup({app:'memex',version:1,ideas:[{id:'safe',text:'x',createdAt:1,doc:{markdown:'x',sources:[{uri:'javascript:alert(1)',title:'bad'}]}}]});return a[0].doc.sources[0].uri==='#';}));
 await p.setViewportSize({width:390,height:844});await p.screenshot({path:require('node:path').join(require('node:os').tmpdir(),'memex-mobile.png'),fullPage:true});
