...
```sh
node tests/study-core.cjs
node tests/study-flow.cjs
```

Both passed during this update. The core suite covers excerpt matching, rejected/missing/duplicate verdicts, answer membership, duplicate distractors, format/Bloom enforcement, and required seed abstraction. The controller suite executes the actual Study controller using simulated DOM/storage/network adapters: separate generation and verification requests, persistence, answer hiding, scoring, rejection without saving, and API error recovery.

JavaScript syntax and HTML resource references were checked. Browser automation was **not run**: Chromium was absent and its download timed out. Responsive layout, real IndexedDB/encryption behavior for the new Study record, file selection, service-worker upgrade, and live provider behavior still need browser verification. No live Gemini key was available; mock successes are not evidence of question quality or OCR accuracy.

Manual browser acceptance:

1. Serve `memex`; configure a key/model. Extract a PDF with columns and equations, correct extraction, and confirm source review is reset after edits.
2. Generate Foundation, Advanced/Technical, mixed-format and seed sets. Confirm explanations cite the reviewed source and seed variants change the scenario while preserving the rule.
3. Interrupt generation; confirm no unverified set appears. Test invalid key, quota exhaustion, unsupported model and offline handling.
4. Answer a set, reload, resume via history, finish, check score/self-assessment, print and retry.
5. Enable encryption with both study and idea data present. Reload/unlock in both modes; confirm raw storage has no source, answers or key in plaintext. Background and return to confirm locking.
6. Switch to Ideas; verify old history, capture, comparison, settings, export and sync. Study sets must not appear as ideas or enter idea sync.
7. Inspect 390px and desktop layouts, keyboard focus, labels, dark/night/custom themes, and returning installed PWA users after service-worker upgrade.

The legacy browser suites below now target `ideas.html`. They were retained and updated for routing, but **not rerun** in this environment. Older pass statements below describe the previous release only.
