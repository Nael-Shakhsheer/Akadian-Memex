...
let settings = { apiKey: '', model: DEFAULT_MODEL, depth: 'standard', searchMode: 'google', ...window.emberInitialSettings };
function saveSettings() { return EmberStorage.saveSettings(settings).catch(e => toast('Settings could not be saved: ' + e.message, 7000)); }
const dbAll = () => EmberStorage.all();
const dbPut = idea => EmberStorage.put(idea);
const dbDel = id => EmberStorage.del(id);
const dbClear = () => EmberStorage.clear();
...
function ideaTools(i) {return `<section class="card idea-tools"><div class="row"><button class="btn small" data-extra="star">${i.favorite?'★ Favorited':'☆ Favorite'}</button><label>Status <select id="ideaStage">${stages.map(s=>`<option ${s===(i.stage||'idea')?'selected':''}>${s}</option>`).join('')}</select></label><button class="btn small" data-extra="edit" ${BUSY.includes(i.status)||chatBusy.has(i.id)?'disabled':''}>Edit idea</button><button class="btn small" data-extra="remind">Revisit in 2 weeks</button>${i.remindAt?'<button class="btn small" data-extra="clearReminder">Clear reminder</button>':''}</div><label class="field">Tags (comma separated)<input id="ideaTags" value="${esc((i.tags||[]).join(', '))}" maxlength="300"></label>${i.remindAt?`<small>Revisit ${esc(fmtWhen(i.remindAt))} · shown when Ember is open</small>`:''}</section>`;}
function chatPanel(i){return `<section class="card chat-panel"><h3>Explore this idea</h3><p class="muted">Follow-ups use the idea and brief as context; answers are not live web research.</p><div class="chat-log">${(i.chat||[]).map(m=>`<div class="chat-turn"><b>${m.role==='user'?'You':'Ember'}</b><div>${md(m.text)}</div></div>`).join('')}</div><form id="chatForm"><label class="field">Ask a follow-up<textarea id="chatInput" required maxlength="8000" placeholder="What would this cost? How could I make it cheaper?"></textarea></label><button class="btn" ${chatBusy.has(i.id)?'disabled':''}>${chatBusy.has(i.id)?'Thinking…':'Ask Ember'}</button></form></section>`;}
const chatBusy=new Set();
function fillExtraSettings(){for(const name of ['depth','searchMode','tavilyKey','gistToken','gistId'])$('#'+name).value=settings[name]||'';$('#lockState').textContent=EmberStorage.lockedEnabled?'Passphrase protection enabled. Lock before leaving this shared device.':'Encryption is off. Enable a passphrase to protect stored keys, ideas and audio.';$('#enableLock').hidden=EmberStorage.lockedEnabled;$('#lockNow').hidden=!EmberStorage.lockedEnabled;}
...
function backup(){return {app:'ember',version:2,ideas:ideas.map(({audio,...i})=>i)};}
async function exportBackup(){try{let data=backup();if(EmberStorage.lockedEnabled){const pass=prompt('Choose a backup passphrase (12+ characters). This protects the exported file; remember it to import elsewhere.');if(pass===null)return;if(pass.length<12)throw new Error('Use at least 12 characters.');data=await EmberStorage.encryptBackup(data,pass);}else if(!confirm('Export an unencrypted backup of your ideas and chats? API keys are excluded.'))return;download(data,'ember-backup-'+new Date().toISOString().slice(0,10)+'.json');}catch(e){toast(e.message,6000);}}
function validateBackup(data){
 if(data?.app!=='ember'||![1,2].includes(data.version)||!Array.isArray(data.ideas)||data.ideas.length>10000)throw new Error('Invalid Ember backup.');
...
async function importBackup(e){const f=e.target.files[0];e.target.value='';if(!f)return;try{if(f.size>20*1024*1024)throw new Error('Backup is too large (20 MB limit).');let data=JSON.parse(await f.text());if(data.encrypted){const pass=prompt('Backup passphrase:');if(pass===null)return;data=await EmberStorage.decryptBackup(data,pass);}validateBackup(data);if(!confirm('Merge this backup? Newer versions replace matching ideas, including deletion records.'))return;toast(`Imported ${await mergeBackup(data)} updates.`);}catch(e){toast('Import failed: '+e.message,7000);}}
let syncing=false;
async function syncGist(){
 ...
 try{
  let snapshot=null;
  if(settings.gistId){const r=await req('/'+settings.gistId);snapshot=await r.json();const f=snapshot.files?.['ember-sync.json'];if(!f||f.truncated)throw new Error('Missing or oversized Ember sync file.');const data=await EmberStorage.decryptBackup(JSON.parse(f.content),pass);await mergeBackup(data);}
  const content=JSON.stringify(await EmberStorage.encryptBackup(backup(),pass));if(content.length>900000)throw new Error('Sync exceeds 900 KB. Export a backup instead.');
  ...
  const result=await (await req(settings.gistId?'/'+settings.gistId:'',{method:settings.gistId?'PATCH':'POST',body:JSON.stringify({description:'Ember encrypted idea sync',public:false,files:{'ember-sync.json':{content}}})})).json();
...
function setupExtras(){
 ...
 if(new URLSearchParams(location.search).get('capture')==='voice'){go(null);$('#thread').insertAdjacentHTML('afterbegin','<section class="card"><h2>Ready to capture</h2><p>Tap the microphone to begin. Your browser may require this tap before recording.</p></section>');$('#micBtn').focus();$('#micBtn').click();}
 document.addEventListener('visibilitychange',()=>{if(document.hidden&&EmberStorage.lockedEnabled){document.body.classList.add('privacy-hidden');}else if(EmberStorage.lockedEnabled&&document.body.classList.contains('privacy-hidden')){location.reload();}});
}
...
(async function init() {
  ...
  if (!settings.apiKey && !welcomed && !new URLSearchParams(location.search).has('capture')) showWelcome();
  setupExtras();
