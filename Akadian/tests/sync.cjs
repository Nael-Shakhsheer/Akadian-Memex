...
(async()=>{
 ...
 async function device(){const c=await browser.newContext();const p=await c.newPage();await c.route('https://api.github.com/gists**',async r=>{const q=r.request();if(q.method()==='GET')return r.fulfill({json:remote});const data=q.postDataJSON();assert(!data.files['memex-sync.json'].content.includes('Sync solar'));remote={id:'01234567890123456789012345678901',files:data.files,history:[{version:String(++revision)}]};return r.fulfill({json:remote});});await p.goto('http://localhost:8765/ideas.html');await p.locator('#introNext').click();await p.locator('#introNext').click();await p.locator('#introNext').click();p.on('dialog',d=>d.accept(d.type()==='prompt'?'shared sync passphrase':undefined));return {c,p};}
