...
3. Commit to the branch served by GitHub Pages. Once deployment completes, close and reopen the app while online. Settings should show **Memex v11.1 · by Akadian**. Reopen again if the old service worker still controls the page.
...
- **Follow-up chat:** questions and answers persist with each idea. Context includes the original idea, the brief (trimmed to 12,000 characters) and recent messages (up to 20 turns / 30,000 characters). Follow-ups do not perform live web searches.
- **Quota and busy handling (11.1):** "high demand" errors are retried every 10 seconds for about five minutes. A per-minute limit is waited out automatically (using Google's suggested delay, up to 75 seconds, twice). A daily allowance cannot be waited out, so the message says which allowance is used and roughly when it resets (midnight Pacific); free-tier quotas are per model, so switching model in Settings gives a fresh allowance. Gemini 3 chat requests ask for low reasoning effort (dropped automatically if a model rejects it). A failed question stays in the box and is not added to the saved history.
