...
window.MemexStorage = (() => {
  ...
  const api={
    ...
    enable(pass,s){return enqueue(async()=>{
      ...
      const study=await seal(await api.study(),k);
      const next={salt,check:await seal('Memex vault v1',k)},config=await seal(s,k);
      await transaction(['ideas','meta'],'readwrite',t=>{const a=t.objectStore('ideas');a.clear();encrypted.forEach(i=>a.put(i));t.objectStore('meta').put(next,'vault');t.objectStore('meta').put(config,'settings');t.objectStore('meta').put(study,'study');});
      meta=next;key=k;clearLocalSettings();
    });},
    async study(){const v=await transaction(['meta'],'readonly',t=>t.objectStore('meta').get('study'));return v?(meta?unseal(v):v):[];},
    saveStudy(value){const snapshot=JSON.parse(JSON.stringify(value));return enqueue(async()=>{const v=meta?await seal(snapshot):snapshot;await write('meta',st=>st.put(v,'study'));});},
...
(async()=>{
  ...
  async function launch(){window.memexInitialSettings=await MemexStorage.settings();gate.close(); const s=document.createElement('script');s.src=document.body.dataset.entry || 'app.js';document.body.append(s);}
  try {
